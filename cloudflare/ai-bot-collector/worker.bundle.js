...
export const EMBEDDED_BOT_PATTERNS = [
    // OpenAI
...
function parseBotLists(raw) {
    try {
        ...
        const referrerDomains = new Set((Array.isArray(body.ai_referrer_domains) ? body.ai_referrer_domains : [])
            .filter((d) => typeof d === "string")
            .map((d) => d.toLowerCase()));
        const verifiableEngines = new Set((Array.isArray(body.verifiable_engines) ? body.verifiable_engines : [])
...
export function classifyRequest(userAgent, referer, lists = EMBEDDED_BOT_LISTS) {
    if (userAgent) {
...
export function isVerifiableBotUA(userAgent, lists = EMBEDDED_BOT_LISTS) {
    if (!userAgent) {
        return false;
    }
    const lower = userAgent.toLowerCase();
...
let botIPCache = null;
// Test hook — isolates the module-level cache between test cases.
export function _resetBotIPCache() {
    botIPCache = null;
}
...
export async function getBotIPRanges(env) {
...
export function parseSignature(raw, label) {
...
export function buildSignatureBase(req, covered, rawParams) {
    const lines = [];
...
export async function verifySignature(request, env) {
    ...
    }, parsed.covered, parsed.rawParams);
    if (base === null) {
    ...
    try {
        ...
        const ok = await crypto.subtle.verify({ name: "Ed25519" }, key, sig, new TextEncoder().encode(base));
...
export async function forwardLog(request, response, env) {
    ...
    const headerSize = Array.from(response.headers.entries()).reduce((total, [key, value]) => total + key.length + value.length + 4, // ": " + "\r\n"
    0);
    const log = {
        timestamp: Date.now(),
 ...
 */
export async function serveWebmcpManifest(request, env, ctx) {
    ...
    const upstreamUrl = buildManifestUrl(env.TRUSTDATA_MANIFEST_URL, env.TRUSTDATA_ATTRIBUTION_ID);
    let upstream;
    try {
        ...
        return new Response(JSON.stringify({ error: "Manifest upstream unreachable" }), { status: 502, headers: { "Content-Type": "application/json" } });
    }
    ...
    if (!upstream.ok) {
        return new Response(JSON.stringify({ error: "Manifest upstream error" }), { status: 502, headers: { "Content-Type": "application/json" } });
    }
    const body = await upstream.text();
    if (env.WEBMCP_CACHE) {
        ctx.waitUntil(env.WEBMCP_CACHE.put(cacheKey, body, {
 ...
 * Exported for tests.
 */
export function buildManifestUrl(template, attributionId) {
